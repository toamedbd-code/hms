...
const _sfc_main = {
  ...
  props: ["bedgroup", "id", "floors"],
  setup(__props) {
    var _a, _b, _c, _d;
    const props = __props;
    const form = useForm({
      name: ((_a = props.bedgroup) == null ? void 0 : _a.name) ?? "",
      floor_id: ((_b = props.bedgroup) == null ? void 0 : _b.floor_id) ?? "",
      description: ((_c = props.bedgroup) == null ? void 0 : _c.description) ?? "",
      _method: ((_d = props.bedgroup) == null ? void 0 : _d.id) ? "put" : "post"
    });
    const submit = () => {
      const routeName = props.id ? route("backend.bedgroup.update", props.id) : route("backend.bedgroup.store");
      form.transform((data) => ({
        ...data,
        remember: "",
        isDirty: false
      ...
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<select id="floor_id" class="block w-full p-1.5 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(form).floor_id) ? ssrLooseContain(unref(form).floor_id, "") : ssrLooseEqual(unref(form).floor_id, "")) ? " selected" : ""}${_scopeId}>Select Floor</option><!--[-->`);
            ssrRenderList(__props.floors, (data) => {
              _push2(`<option${ssrRenderAttr("value", data.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).floor_id) ? ssrLooseContain(unref(form).floor_id, data.id) : ssrLooseEqual(unref(form).floor_id, data.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(data.name)}</option>`);
            });
            _push2(`<!--]--></select>`);
            _push2(ssrRenderComponent(_sfc_main$3, {
              class: "mt-2",
            ...
            _push2(`<textarea id="description" class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600" type="text" placeholder=""${_scopeId}>${ssrInterpolate(unref(form).description)}</textarea>`);
            _push2(ssrRenderComponent(_sfc_main$3, {
              class: "mt-2",
              message: unref(form).errors.description
            }, null, _parent2, _scopeId));
                  ...
                  createVNode("div", { class: "p-2 py-2 flex items-center space-x-2" }, [
                    createVNode("div", { class: "flex items-center space-x-3" }, [
                      createVNode("button", {
                        ...
                        createTextVNode(" Bed Group List ")
                      ])
                    ])
                  ])
                ]),
                  ...
                  createVNode("div", { class: "grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4" }, [
                    ...
                    createVNode("div", { class: "col-span-1 md:col-span-1" }, [
                      ...
                      withDirectives(createVNode("select", {
                        id: "floor_id",
                        "onUpdate:modelValue": ($event) => unref(form).floor_id = $event,
                        ...
                        createVNode("option", { value: "" }, "Select Floor"),
                        (openBlock(true), createBlock(Fragment, null, renderList(__props.floors, (data) => {
                        ...
                        [vModelSelect, unref(form).floor_id]
                      ]),
                      createVNode(_sfc_main$3, {
                        class: "mt-2",
                        message: unref(form).errors.description
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Backend/BedGroup/Form.vue");
