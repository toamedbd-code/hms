...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const props = __props;
    const form = useForm({
      name: ((_a = props.specialist) == null ? void 0 : _a.name) ?? "",
      _method: ((_b = props.specialist) == null ? void 0 : _b.id) ? "put" : "post"
    });
    const submit = () => {
      const routeName = props.id ? route("backend.specialist.update", props.id) : route("backend.specialist.store");
      form.transform((data) => ({
        ...data,
        remember: "",
        isDirty: false
                  ...
                  createVNode("div", { class: "p-2 py-2 flex items-center space-x-2" }, [
                    createVNode("div", { class: "flex items-center space-x-3" }, [
                      createVNode("button", {
                        ...
                        createTextVNode(" Specialist List ")
                      ])
                    ])
                  ])
                ]),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Backend/Specialist/Form.vue");
