...
const _sfc_main = {
  ...
  props: ["chargecategory", "id", "chargeTypes"],
  setup(__props) {
    var _a, _b, _c, _d;
    const props = __props;
    const form = useForm({
      charge_type_id: ((_a = props.chargecategory) == null ? void 0 : _a.charge_type_id) ?? "",
      name: ((_b = props.chargecategory) == null ? void 0 : _b.name) ?? "",
      description: ((_c = props.chargecategory) == null ? void 0 : _c.description) ?? "",
      _method: ((_d = props.chargecategory) == null ? void 0 : _d.id) ? "put" : "post"
    });
    const submit = () => {
      const routeName = props.id ? route("backend.chargecategory.update", props.id) : route("backend.chargecategory.store");
      form.transform((data) => ({
        ...data,
        remember: "",
        isDirty: false
      ...
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<select id="charge_type_id" class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(form).charge_type_id) ? ssrLooseContain(unref(form).charge_type_id, "") : ssrLooseEqual(unref(form).charge_type_id, "")) ? " selected" : ""}${_scopeId}>Select Charge Type</option><!--[-->`);
            ssrRenderList(__props.chargeTypes, (chargeType) => {
              _push2(`<option${ssrRenderAttr("value", chargeType.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).charge_type_id) ? ssrLooseContain(unref(form).charge_type_id, chargeType.id) : ssrLooseEqual(unref(form).charge_type_id, chargeType.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(chargeType.name)}</option>`);
            });
            _push2(`<!--]--></select>`);
            ...
            _push2(`<input id="name" class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600"${ssrRenderAttr("value", unref(form).name)} type="text" placeholder="Enter Name"${_scopeId}>`);
            _push2(ssrRenderComponent(_sfc_main$3, {
              class: "mt-2",
              message: unref(form).errors.name
            }, null, _parent2, _scopeId));
            _push2(`</div><div class="col-span-2 md:col-span-2"${_scopeId}>`);
            _push2(ssrRenderComponent(_sfc_main$2, {
              for: "description",
              value: "Description *"
            }, null, _parent2, _scopeId));
            _push2(`<textarea id="description" class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600" rows="2" placeholder="Enter Description"${_scopeId}>${ssrInterpolate(unref(form).description)}</textarea>`);
            _push2(ssrRenderComponent(_sfc_main$3, {
              class: "mt-2",
                ...
                createVNode("div", { class: "flex items-center justify-between w-full text-gray-700 bg-gray-100 rounded-md" }, [
                  ...
                  createVNode("div", { class: "p-2 py-2 flex items-center space-x-2" }, [
                    createVNode("div", { class: "flex items-center space-x-3" }, [
                      createVNode("button", {
                        onClick: goToChargeCategoryList,
                        ...
                        createTextVNode(" Charge Category List ")
                      ])
                    ])
                  ])
                ]),
                  ...
                  createVNode("div", { class: "grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4" }, [
                    createVNode("div", { class: "col-span-1 md:col-span-1" }, [
                      ...
                      withDirectives(createVNode("select", {
                        ...
                        (openBlock(true), createBlock(Fragment, null, renderList(__props.chargeTypes, (chargeType) => {
                          return openBlock(), createBlock("option", {
                            key: chargeType.id,
                            value: chargeType.id
                          }, toDisplayString(chargeType.name), 9, ["value"]);
                        }), 128))
                    ...
                    createVNode("div", { class: "col-span-2 md:col-span-2" }, [
                      createVNode(_sfc_main$2, {
                        for: "description",
                        value: "Description *"
                      }),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Backend/ChargeCategory/Form.vue");
