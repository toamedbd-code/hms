...
const _sfc_main = {
  ...
  props: ["medicinecompany", "id"],
  setup(__props) {
    var _a, _b;
    const props = __props;
    const form = useForm({
      name: ((_a = props.medicinecompany) == null ? void 0 : _a.name) ?? "",
      _method: ((_b = props.medicinecompany) == null ? void 0 : _b.id) ? "put" : "post"
    });
    const submit = () => {
      const routeName = props.id ? route("backend.medicinecompany.update", props.id) : route("backend.medicinecompany.store");
      form.transform((data) => ({
        ...data,
        remember: "",
        isDirty: false
                ...
                createVNode("div", { class: "flex items-center justify-between w-full text-gray-700 bg-gray-100 rounded-md" }, [
                  ...
                  createVNode("div", { class: "p-2 py-2 flex items-center space-x-2" }, [
                    createVNode("div", { class: "flex items-center space-x-3" }, [
                      createVNode("button", {
                        onClick: goToMedicineCompanyList,
                        ...
                        createTextVNode(" Medicine Company List ")
                      ])
                    ])
                  ])
                ]),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Backend/MedicineCompany/Form.vue");
