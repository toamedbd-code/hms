<script setup>
import BackendLayout from '@/Layouts/BackendLayout.vue';
import { Link } from '@inertiajs/vue3';
import { ref, computed, onMounted } from 'vue';
import axios from 'axios';

const loading = ref(false);
const filters = ref({
  from: '',
  to: new Date().toISOString().slice(0, 10),
});

const incomeRows = ref([]);
const expenseRows = ref([]);
const totals = ref({ income: 0, expense: 0, net_profit: 0 });

const isProfit = computed(() => Number(totals.value.net_profit || 0) >= 0);

function formatAmount(v) {
  return Number(v || 0).toFixed(2);
}

async function loadReport() {
  loading.value = true;
  try {
    const res = await axios.get(route('backend.accounts.profit-loss.list'), {
      params: {
        from: filters.value.from || undefined,
        to: filters.value.to || undefined,
      },
    });

    incomeRows.value = res?.data?.income_rows || [];
    expenseRows.value = res?.data?.expense_rows || [];
    totals.value = res?.data?.totals || { income: 0, expense: 0, net_profit: 0 };
  } catch (err) {
    console.error(err);
    incomeRows.value = [];
    expenseRows.value = [];
    totals.value = { income: 0, expense: 0, net_profit: 0 };
  } finally {
    loading.value = false;
  }
}

function resetFilters() {
  filters.value.from = '';
  filters.value.to = new Date().toISOString().slice(0, 10);
  loadReport();
}

function exportCsv() {
  const url = route('backend.accounts.profit-loss.list', {
    from: filters.value.from || undefined,
    to: filters.value.to || undefined,
    format: 'csv',
  });

  window.open(url, '_blank', 'noopener');
}

onMounted(() => {
  loadReport();
});
</script>

<template>
  <BackendLayout>
    <div class="w-full p-4 bg-white rounded-md dark:bg-slate-900">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 class="text-xl font-bold">{{ $page.props.pageTitle }}</h1>
          <p class="text-sm text-gray-600 dark:text-gray-300">Income vs Expense summary for a selected period.</p>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          <Link :href="route('backend.accounts.index')" class="px-3 py-2 text-sm rounded border border-slate-300 dark:border-slate-600">Chart of Accounts</Link>
          <Link :href="route('backend.accounts.trial-balance')" class="px-3 py-2 text-sm rounded border border-slate-300 dark:border-slate-600">Trial Balance</Link>
          <Link :href="route('backend.accounts.balance-sheet')" class="px-3 py-2 text-sm rounded border border-slate-300 dark:border-slate-600">Balance Sheet</Link>
          <Link :href="route('backend.accounts.cash-flow')" class="px-3 py-2 text-sm rounded border border-slate-300 dark:border-slate-600">Cash Flow</Link>
        </div>
      </div>

      <div class="mt-4 p-3 border rounded bg-gray-50 dark:bg-slate-800/40 dark:border-slate-700">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div>
            <label class="text-sm">From Date</label>
            <input v-model="filters.from" type="date" class="w-full border rounded p-2 dark:bg-slate-700 dark:border-slate-600" />
          </div>
          <div>
            <label class="text-sm">To Date</label>
            <input v-model="filters.to" type="date" class="w-full border rounded p-2 dark:bg-slate-700 dark:border-slate-600" />
          </div>
          <div class="flex items-end gap-2">
            <button @click="loadReport" class="btn-colorful-sm">Apply</button>
            <button @click="resetFilters" class="px-3 py-2 text-sm rounded border border-slate-300 dark:border-slate-600">Reset</button>
            <button @click="exportCsv" class="px-3 py-2 text-sm rounded bg-emerald-600 text-white">Export CSV</button>
          </div>
        </div>
      </div>

      <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
        <div class="p-3 rounded border dark:border-slate-700">
          <div class="text-gray-500">Total Income</div>
          <div class="text-lg font-semibold">{{ formatAmount(totals.income) }}</div>
        </div>
        <div class="p-3 rounded border dark:border-slate-700">
          <div class="text-gray-500">Total Expense</div>
          <div class="text-lg font-semibold">{{ formatAmount(totals.expense) }}</div>
        </div>
        <div class="p-3 rounded border dark:border-slate-700" :class="isProfit ? 'border-emerald-500' : 'border-amber-500'">
          <div class="text-gray-500">Net {{ isProfit ? 'Profit' : 'Loss' }}</div>
          <div class="text-lg font-semibold" :class="isProfit ? 'text-emerald-600' : 'text-amber-600'">{{ formatAmount(totals.net_profit) }}</div>
        </div>
      </div>

      <div class="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="border rounded dark:border-slate-700 overflow-x-auto">
          <div class="px-4 py-2 font-semibold bg-gray-50 dark:bg-slate-800">Income</div>
          <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            <thead class="bg-gray-50 dark:bg-slate-800">
              <tr>
                <th class="px-4 py-2 text-left">Code</th>
                <th class="px-4 py-2 text-left">Name</th>
                <th class="px-4 py-2 text-right">Amount</th>
              </tr>
            </thead>
            <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-700">
              <tr v-for="row in incomeRows" :key="`inc-${row.account_id || row.code}`">
                <td class="px-4 py-2">{{ row.code }}</td>
                <td class="px-4 py-2">{{ row.name }}</td>
                <td class="px-4 py-2 text-right">{{ formatAmount(row.amount) }}</td>
              </tr>
              <tr v-if="!loading && incomeRows.length === 0"><td class="px-4 py-6 text-center text-gray-500" colspan="3">No income rows</td></tr>
            </tbody>
          </table>
        </div>

        <div class="border rounded dark:border-slate-700 overflow-x-auto">
          <div class="px-4 py-2 font-semibold bg-gray-50 dark:bg-slate-800">Expenses</div>
          <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            <thead class="bg-gray-50 dark:bg-slate-800">
              <tr>
                <th class="px-4 py-2 text-left">Code</th>
                <th class="px-4 py-2 text-left">Name</th>
                <th class="px-4 py-2 text-right">Amount</th>
              </tr>
            </thead>
            <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-700">
              <tr v-for="row in expenseRows" :key="`exp-${row.account_id || row.code}`">
                <td class="px-4 py-2">{{ row.code }}</td>
                <td class="px-4 py-2">{{ row.name }}</td>
                <td class="px-4 py-2 text-right">{{ formatAmount(row.amount) }}</td>
              </tr>
              <tr v-if="!loading && expenseRows.length === 0"><td class="px-4 py-6 text-center text-gray-500" colspan="3">No expense rows</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    </div>
  </BackendLayout>
</template>

<style scoped></style>
