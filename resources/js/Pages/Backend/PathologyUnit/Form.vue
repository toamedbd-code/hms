...
const props = defineProps(['pathologyunit', 'id']);

const form = useForm({
    name: props.pathologyunit?.name ?? '',

    _method: props.pathologyunit?.id ? 'put' : 'post',
});
...
const goBack = () => {
    ...
    router.visit(route('backend.pathologyunit.index'));
};
