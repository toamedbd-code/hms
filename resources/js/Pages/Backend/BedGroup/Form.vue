...
const props = defineProps(['bedgroup', 'id', 'floors']);

const form = useForm({
    name: props.bedgroup?.name ?? '',
    floor_id: props.bedgroup?.floor_id ?? '',
    description: props.bedgroup?.description ?? '',

    _method: props.bedgroup?.id ? 'put' : 'post',
});
