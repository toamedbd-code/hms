...
const props = defineProps(['medicinedosage', 'id', 'medicineCatgories', 'medicineUnits']);

const form = useForm({
    medicine_category_id: props.medicinedosage?.medicine_category_id ?? '',
    dose: props.medicinedosage?.dose ?? '',
    medicine_unit_id: props.medicinedosage?.medicine_unit_id ?? '',

    _method: props.medicinedosage?.id ? 'put' : 'post',
});
            ...
            <form @submit.prevent="submit" class="p-4">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="col-span-1 md:col-span-1">
                        <InputLabel for="medicine_category_id" value="Medicine Category Name" />
                        <select v-model="form.medicine_category_id"
                            ...
                            <option v-for="data in medicineCatgories" :key="data.id" :value="data.id">{{ data.name }}
                            </option>
                        </select>
                        <InputError class="mt-2" :message="form.errors.medicine_category_id" />
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-1">
                        ...
                        <input id="dose"
                            ...
                            v-model="form.dose" type="text" placeholder="Dose" />
                        <InputError class="mt-2" :message="form.errors.dose" />
                    </div>

                    <div class="col-span-1 md:col-span-1">
                        <InputLabel for="medicine_unit_id" value="Unit" />
                        <select v-model="form.medicine_unit_id"
                            class="w-full p-2 text-sm bg-white rounded shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600">
                            <option value="">Select Medicine Unit</option>
                            <option v-for="data in medicineUnits" :key="data.id" :value="data.id">{{ data.name }}
                            </option>
                        </select>
                        <InputError class="mt-2" :message="form.errors.medicine_unit_id" />
                    </div>
