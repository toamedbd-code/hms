...
const props = defineProps(['medicinesupplier', 'id']);

const form = useForm({
    name: props.medicinesupplier?.name ?? '',
    phone: props.medicinesupplier?.phone ?? '',
    contact_person_name: props.medicinesupplier?.contact_person_name ?? '',
    contact_person_phone: props.medicinesupplier?.contact_person_phone ?? '',
    drug_lisence_no: props.medicinesupplier?.drug_lisence_no ?? '',
    address: props.medicinesupplier?.address ?? '',
    status: props.medicinesupplier?.status ?? '',

    _method: props.medicinesupplier?.id ? 'put' : 'post',
});
        ...
        <div class="w-full transition duration-1000 ease-in-out transform bg-white rounded-md">
            ...
            <form @submit.prevent="submit" class="p-4">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="col-span-1 md:col-span-1">
                        <InputLabel for="drug_lisence_no" value="Drug Lisence Number" />
                        <input id="drug_lisence_no"
                            class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600"
                            v-model="form.drug_lisence_no" type="text" placeholder="Drug Lisence Number" />
                        <InputError class="mt-2" :message="form.errors.drug_lisence_no" />
                    </div>
                    <div class="col-span-1 md:col-span-1">
