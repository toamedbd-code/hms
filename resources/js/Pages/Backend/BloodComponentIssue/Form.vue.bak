...
const props = defineProps(['bloodcomponentissue', 'id', 'patients', 'doctors']);

const form = useForm({
    case_id: props.bloodcomponentissue?.case_id ?? '',
    patient_id: props.bloodcomponentissue?.patient_id ?? '',
    issue_date: props.bloodcomponentissue?.issue_date ?? '',
    doctor_id: props.bloodcomponentissue?.doctor_id ?? '',
    reference_name: props.bloodcomponentissue?.reference_name ?? '',
    technician: props.bloodcomponentissue?.technician ?? '',
    blood_group: props.bloodcomponentissue?.blood_group ?? '',
    bag: props.bloodcomponentissue?.bag ?? '',
    charge_category: props.bloodcomponentissue?.charge_category ?? '',
    charge_name: props.bloodcomponentissue?.charge_name ?? '',
    standard_charge: props.bloodcomponentissue?.standard_charge ?? 0,
    note: props.bloodcomponentissue?.note ?? '',
    total: props.bloodcomponentissue?.total ?? 0,
    discount: props.bloodcomponentissue?.discount ?? 0,
    discount_percent: props.bloodcomponentissue?.discount_percent ?? 0,
    tax: props.bloodcomponentissue?.tax ?? 0,
    tax_percent: props.bloodcomponentissue?.tax_percent ?? 0,
    net_amount: props.bloodcomponentissue?.net_amount ?? 0,
    payment_mode: props.bloodcomponentissue?.payment_mode ?? 'Cash',
    payment_amount: props.bloodcomponentissue?.payment_amount ?? 0,
    apply_tpa: Boolean(props.bloodcomponentissue?.apply_tpa) || false,

    _method: props.bloodcomponentissue?.id ? 'put' : 'post',
});
...
<template>
    <BackendLayout>
        <!-- Patient Modal -->
        <PatientModal :isOpen="isPatientModalOpen" :tpas="props.tpas" @close="closePatientModal" @patientCreated="handlePatientCreated" />
            ...
            <div
                ...
                <div class="p-2 py-2 flex items-center space-x-2">
                    <div class="flex items-center space-x-3">
                        <button @click="goToBloodComponentList"
            ...
            <form @submit.prevent="submit" class="spaced-form">
                ...
                <!-- First Row: Patient Selection, Case ID, Apply TPA -->
                    ...
                    <div class="col-span-8 space-y-2">
                        ...
                        <div class="flex justify-between items-center py-1">
                            ...
                            <div class="flex items-center gap-2">
                                <input v-model="form.discount_percent" type="number" step="1" placeholder="0"
                                    @input="handleDiscountPercentChange"
                                    class="w-12 px-1 py-1 border border-gray-300 rounded text-xs text-center" />
                                <span class="text-xs">%</span>
                                <span class="text-sm font-semibold min-w-[60px] text-right">{{ (parseFloat(form.discount) || 0).toFixed(2)
                                    }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center py-1">
                            ...
                            <div class="flex items-center gap-2">
                                <input v-model="form.tax_percent" type="number" step="1" placeholder="0"
                                    @input="handleTaxPercentChange"
                                    class="w-12 px-1 py-1 border border-gray-300 rounded text-xs text-center" />
                                <span class="text-xs">%</span>
                                <span class="text-sm font-semibold min-w-[60px] text-right">{{ (parseFloat(form.tax) || 0).toFixed(2)
                                    }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center py-2 border-t border-gray-300">
                            <span class="text-sm font-semibold text-black">Net Amount (Tk.)</span>
