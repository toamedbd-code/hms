...
const props = defineProps(["defineleave", "id", "leaveDetails", "roleDetails"]);

const form = useForm({
  role_id: props.defineleave?.role_id ?? '',
  type_id: props.defineleave?.type_id ?? '',
  days: props.defineleave?.days ?? '',
  date: props.defineleave?.date ?? '', 
  imagePreview: props.defineleave?.image ?? "",
  filePreview: props.defineleave?.file ?? "",
  _method: props.defineleave?.id ? "put" : "post",
});
...
const submit = () => {
  const routeName = props.id
    ? route("backend.defineleave.update", props.id)
    : route("backend.defineleave.store");
  form
    .transform((data) => ({
      ...data,
      remember: "",
        ...
        <div class="flex items-center p-3 py-2 space-x-1">
          <div class="p-2 py-2 flex items-center space-x-2">
            <div class="flex items-center space-x-3">
              <button @click="goToDefineLeaveList"
